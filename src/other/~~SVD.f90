program aprox
    REAL A, U, V, SIGMA, WORK, C, RELEER, SIGMA1, TAU, Y
    INTEGER I, IERR, J, M, N, NM, K, T, L, P
    ALLOCATABLE A(:, :, :), U(:, :), V(:, :), WORK(:), C(:), SIGMA(:), Y(:, :)
    READ(*, *) K
    READ(*, *) M ! Ввод колличества точек
    READ(*, *) N ! Ввод колличества коэффициентов (столбцов для А)
    ALLOCATE (A(K, M, N), U(M, N), V(M, N), WORK(N), C(N), SIGMA(N), Y(K, M))
    NM = M
    RELEER = 0.000001
    !       !Ввод точек ординаты (Y - напряжение)
    DO 5 L = 1, K
        DO 10 I = 1, M
            READ(*, *) Y(L, I)
        10      CONTINUE
        !       ! Ввод значений матрицы плана
        DO 20 P = 1, M
            DO 25 J = 1, N
                READ(*, *) A(L, P, J)
            25         CONTINUE
    ...
    !        A(8,3) = 0.39204E+07
    DO 7 T = 1, K
        CALL SVD(NM, M, N, A(T, :, :), SIGMA, .TRUE., U, .TRUE., V, IERR, WORK)
        !        PRINT*, IERR
        !        SIGMA(1) = 1
        !        SIGMA(2) = 2
        !       SIGMA(3) = 3
        ...
        PRINT*, C
    7       CONTINUE
    DEALLOCATE (A, U, V, WORK, C, SIGMA, Y)
    pause
end program aprox
...
SUBROUTINE SVD(NM, M, N, A, W, MATU, U, MATV, V, IERR, RV1)
    ...
    !     *  IERR
    !           Attention, Otladka !: Perewozu iz form.par.
    !           wse massiwi A,U,V,W,RV1   =>   w Common !
    !           + int=>int*2, real=>real*4
    !           + isklyuchayu NM iz opisaniya i form.par.tolko.
    !
    REAL A(NM, N), W(N), U(NM, N), V(NM, N), RV1(N)
    REAL C, F, G, H, S, X, Y, Z, SCALE, ANORM
    LOGICAL MATU, MATV
    !        common /svd12/ A,U,V,W,RV1
    IERR = 0
    DO 100 I = 1, M
        DO 100 J = 1, N
            U(I, J) = A(I, J)
