...
10      CONTINUE
        ...
        DO 21 I = 1, M_OBSH
           ...
           diff = 0.
           m_begin = I
           m_end =I + windows ! максимальное кол-во эл массива для одной итерации, этот массив есть окно которое двигается по всему массиву наблюдений, двигается с шагом 1
           t_nul = I + (windows/2.)
        ...
        CALL SVD(windows,windows,N,A,SIGMA,.TRUE.,U,.TRUE.,V,IERR, WORK)
            ...
            ENDIF  !PRINT*, SIGMA
        CALL CALC_COEFF(Y(m_begin:m_end),U,V,SIGMA,C,windows,N,RELEER) ! Y(m_begin:m_end) вырезка окна из массива наблюдений (ось напряжения)
